...
This directory houses AIIR-native commitment receipts and Sigstore bundle sidecars for the cleaned NISQ benchmarking evidence-companion entry and the full paper-facing capsule mirror.

Current bundle set:

- `receipts/receipt_nisq-paper-capsule_*.json` binds `capsule/`, the full 82-file public evidence capsule mirror copied from the published allowlist. This is the primary paper-referenceable receipt in the sidecar repo.
- `receipts/receipt_nisq-paper-sidecar_*.json` binds the public packet files `README.md`, `COMMITMENT.json`, `SHA256SUMS`, and the copied `source/` directory.
- `receipts/receipt_nisq-zenodo-v2026-05-02_*.json` binds the exact published Zenodo tarball digest `sha256:e49279b120a9e2a7e567293addafa55e09d26161b3bc17243e550e2a6a41690c`.
...
- `BUNDLESET.json` is the authoritative index for the current receipt filenames, artifact digests, signer policy, and Rekor-backed transparency metadata.
...
- The current signed state is recorded in `BUNDLESET.json` for exact receipt coordinates, artifact digests, certificate hashes, and verification metadata.

Verification:

```bash
cd commitments/nisq-benchmarking-evidence-2026-05-02
sha256sum -c aiir/SHA256SUMS

PYTHONPATH=/path/to/aiir /usr/bin/env python3 -m aiir \
  --verify aiir/receipts/receipt_nisq-paper-capsule_*.json \
  --verify-signature \
  --signer-identity noah@invariantsystems.io \
  --signer-issuer https://accounts.google.com

PYTHONPATH=/path/to/aiir /usr/bin/env python3 -m aiir \
  --verify aiir/receipts/receipt_nisq-paper-sidecar_*.json \
  --verify-signature \
  --signer-identity noah@invariantsystems.io \
  --signer-issuer https://accounts.google.com

PYTHONPATH=/path/to/aiir /usr/bin/env python3 -m aiir \
  --verify aiir/receipts/receipt_nisq-zenodo-v2026-05-02_*.json \
  --verify-signature \
  --signer-identity noah@invariantsystems.io \
  --signer-issuer https://accounts.google.com
